...
while 1:
    ...
    #roll = math.atan(ay / math.sqrt(ax * ax + az * az)) * 57.324
    #pitch = math.atan(ax / math.sqrt( ay* ay+ az*az ) ) * 57.324

    #pitch = math.atan(-ax / (ay*math.sin(roll) + az*math.cos(roll)))

    deg_theta,deg_psi,deg_phi = getDeg(ax,ay,az)
    print('theta = {0:4.3f},psi = {1:4.3f},phi = {2:4.3f}' .format(deg_theta,deg_psi,deg_phi))
    sleep(0.2)
